...
mod costs {
    ...
    /// Cost of the Berlin alt_bn128_add operation.
    pub(super) const BERLIN_ADD: EthGas = EthGas::new(150);

    /// Cost of the Berlin alt_bn128_mul operation.
    pub(super) const BERLIN_MUL: EthGas = EthGas::new(6_000);

    /// Cost of the alt_bn128_pair per point.
    pub(super) const BERLIN_PAIR_PER_POINT: EthGas = EthGas::new(34_000);

    /// Cost of the alt_bn128_pair operation.
    pub(super) const BERLIN_PAIR_BASE: EthGas = EthGas::new(45_000);
}
...
impl<HF: HardFork> Bn128Mul<HF> {
    fn run_inner(input: &[u8], _context: &Context) -> Result<Vec<u8>, ExitError> {
        ...
        if let Some(mul) = AffineG1::from_jacobian(p * fr) {
            mul.x().to_big_endian(&mut output[0..32]).unwrap();
            mul.y().to_big_endian(&mut output[32..64]).unwrap();
        }
...
impl<HF: HardFork> Bn128Pair<HF> {
    fn run_inner(input: &[u8], _context: &Context) -> Result<Vec<u8>, ExitError> {
        ...
        output.to_big_endian(&mut result);
...
mod tests {
    ...
    fn test_alt_bn128_pair() {
        ...
        let res = Bn128Pair::<Berlin>::new()
            .run(&input, Some(EthGas::new(113_000)), &new_context(), false)
            .unwrap()
            .output;
        assert_eq!(res, expected);
        ...
        .unwrap();
        let res = Bn128Pair::<Berlin>::new().run(
            &input,
            Some(EthGas::new(112_999)),
            &new_context(),
            false,
        );
        assert!(matches!(res, Err(ExitError::OutOfGas)));
